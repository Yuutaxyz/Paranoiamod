...
package net.mcreator.paranoiamod.world.structure;
...
public class CemiteriomenorStructure extends ParanoiamodModElements.ModElement {
	public CemiteriomenorStructure(ParanoiamodModElements instance) {
		super(instance, 31);
	}
		...
		Feature<NoFeatureConfig> feature = new Feature<NoFeatureConfig>(NoFeatureConfig::deserialize) {
			...
			public boolean place(IWorld world, ChunkGenerator generator, Random random, BlockPos pos, NoFeatureConfig config) {
				...
				if (!dimensionCriteria)
					return false;
				if ((random.nextInt(1000000) + 1) <= 8688) {
					...
					for (int a = 0; a < count; a++) {
						...
						Template template = ((ServerWorld) world.getWorld()).getSaveHandler().getStructureTemplateManager()
								.getTemplateDefaulted(new ResourceLocation("paranoiamod", "cemiterio"));
						if (template == null)
							return false;
