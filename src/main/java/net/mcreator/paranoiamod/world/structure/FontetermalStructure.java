...
public class FontetermalStructure extends ParanoiamodModElements.ModElement {
	public FontetermalStructure(ParanoiamodModElements instance) {
		super(instance, 37);
	}
		...
		Feature<NoFeatureConfig> feature = new Feature<NoFeatureConfig>(NoFeatureConfig::deserialize) {
			...
			public boolean place(IWorld world, ChunkGenerator generator, Random random, BlockPos pos, NoFeatureConfig config) {
				...
				if (!dimensionCriteria)
					return false;
				if ((random.nextInt(1000000) + 1) <= 9976) {
					...
					for (int a = 0; a < count; a++) {
						...
						Template template = ((ServerWorld) world.getWorld()).getSaveHandler().getStructureTemplateManager()
								.getTemplateDefaulted(new ResourceLocation("paranoiamod", "fontetermal"));
						if (template == null)
							return false;
